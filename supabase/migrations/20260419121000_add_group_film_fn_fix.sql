-- SBS-26 (fix): renomme les OUT parameters pour éviter un conflit
-- "column reference id is ambiguous" entre la colonne group_films.id
-- et le paramètre de sortie du même nom.

drop function if exists public.add_group_film(integer);

create or replace function public.add_group_film(p_tmdb_id integer)
returns table(film_id uuid, already_exists boolean)
language plpgsql
security definer
